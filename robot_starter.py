...
pid_pan = PID.PID(0.5, 0.65, 0.0575) # TODO Create a PID controller object for the pan servo
pid_tilt = PID.PID(0.6, 0.35, 0.045) # TODO Create a PID controller object for the tilt servo
...
def get_face_center(face):
    return (face[0] + face[2] // 2, face[1] + face[3] // 2) # TODO Replace temporary return value

def get_difference(face_mid):
    return math.sqrt((face_mid[0]-image_center[0])**2 + (face_mid[1]-image_center[1])**2)
...
def get_centermost_face(face_list):
    ...
    if len(face_list) == 0:
        ...
        face = []
        for x in face_list:
            diff = get_difference(get_face_center(x))
            if t > diff:
                t = diff
                face = x
        return face
...
def do_motion(faces):
    ...
    best_face_mid = get_face_center(best_face)

    if get_difference(best_face_mid) < best_face[2] / 5:
        return

    # TODO Update the pan PID controller
    pid_pan.SetPoint = image_center[0]
    pid_pan.update(best_face_mid[0])
    motor_pan = clamp(int(pid_pan.output) + cur_pan, bounds_pan)

    # TODO Update the tilt PID controller
    pid_tilt.SetPoint = image_center[1]
    pid_tilt.update(best_face_mid[1])
    motor_tilt = clamp(int(pid_tilt.output) + cur_tilt, bounds_tilt)

    # print("pan: ",cur_pan, pid_pan.output, motor_pan, ", tilt: ", cur_tilt, pid_tilt.output, motor_tilt)
    # TODO Set the motor positions
    cur_pan = motor_pan
    cur_tilt = motor_tilt
    set_pan(cur_pan, 100)
    set_tilt(cur_tilt, 100)
...
def face_detection(stop_sig, img_lock, face_cond):
    ...
    while not stop_sig.is_set():
        ...
        try:
            ...
            with face_cond:
                # Do face detection
                temp_faces = classifier.detectMultiScale(g_img, scaleFactor=1.1, minNeighbors=6) # Replace this line with the actual face detection
